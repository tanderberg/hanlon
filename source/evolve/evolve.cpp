/* evolve.cpp : Euler, RK4 and symplectic (default) integrators (Minkowski).
 * 
...
#pragma omp for
       ...
       for (int i = 0; i <= nSideM1; i++)
           for (int j = 0; j <= nSideM1; j++)
               for (int k = 0; k <= nSideM1; k++)
               {
                   compute_dotFields_and_dDotFields_dFields(&(fields[scratch][i][j][k]), &(momenta[active][i][j][k]), df, A);
...
#pragma omp for
       ...
       for (int i = 0; i <= nSideM1; i++)
           for (int j = 0; j <= nSideM1; j++)
               for (int k = 0; k <= nSideM1; k++)
               {
                   compute_dotMomenta_and_dDotFields_dFields(i, j, k, fields[scratch], &(momenta[from_p][i][j][k]), dm, A);
