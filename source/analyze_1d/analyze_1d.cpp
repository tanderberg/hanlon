/* analyze_1d.cpp : Analyzes raw (cartesian) field data (1D lattice).
 * 
...
TFields dotFields[MAX_NSIDE];   /* Max allowed number of fields always allocated. Ugly but fast. */
...
public:
   ...
   double get_rho_static_core(TFields *fields, double h_00, double h_01, double h_02, double h_11, double h_12, double h_22, 
                              double dx_ph_0, double dx_ph_1, double dx_ph_2, double dx_ph_3, 
   ...
   double compute_q(TFields *fields, TFields *dotFields,
                    double h_00, double h_01, double h_02, double h_11, double h_12, double h_22, 
                    double dx_ph_0, double dx_ph_1, double dx_ph_2, double dx_ph_3);
   void compute_observables(int i, TFields fields[MAX_NSIDE], TFields dotFields[MAX_NSIDE], TObservable& o);
};
...
/* Returns matrix with squared photon masses. */
{
   ...
   double dth[2][4]; // First index is Lorentz (0 = time), second is group

   dth[0][0] = dotFields->ph_0;
   dth[0][1] = dotFields->ph_1;
   dth[0][2] = dotFields->ph_2;
   dth[0][3] = dotFields->ph_3;
   ...
   for (int a = 0; a < 4; a++)
       for (int b = 0; b < 4; b++)
       {
          M_11 += h[a][b]*dth[0][a]*dth[0][b];
          M_22 += h[a][b]*(dth[0][a]*dth[0][b] + dth[1][a]*dth[1][b]);
          M_33 += h[a][b]*(dth[0][a]*dth[0][b] + dth[1][a]*dth[1][b]);
       }
}

double TAnalyzeLattice::get_rho_static_core(TFields *fields, 
                                            double h_00, double h_01, double h_02, double h_11, double h_12, double h_22, 
                                            double dx_ph_0, double dx_ph_1, double dx_ph_2, double dx_ph_3, 
                                            double dx_A_2, double dx_A_3,
                                            TRho &rho)
/* Individual terms are returned in the TRho struct.  */
{
   ...
   AcrossGradPh_am[0][0] = 0;
   AcrossGradPh_am[0][1] = fields->A_3*dx_ph_0;
   AcrossGradPh_am[0][2] = -fields->A_2*dx_ph_0;
   
   AcrossGradPh_am[1][0] = 0;
   AcrossGradPh_am[1][1] = fields->A_3*dx_ph_1;
   AcrossGradPh_am[1][2] = -fields->A_2*dx_ph_1;
   
   AcrossGradPh_am[2][0] = 0;
   AcrossGradPh_am[2][1] = fields->A_3*dx_ph_2;
   AcrossGradPh_am[2][2] = -fields->A_2*dx_ph_2;
   
   AcrossGradPh_am[3][0] = 0;
   AcrossGradPh_am[3][1] = fields->A_3*dx_ph_3;
   AcrossGradPh_am[3][2] = -fields->A_2*dx_ph_3;

   rho.BR2 = ( pow(dx_A_3, 2) + pow(dx_A_2, 2) ) / 2;
   ...
   if (0 > rho.rH) rho.rH = 0; else rho.rH /= 2;

   rho.rG = (  dx_ph_0*dx_ph_0
             + dx_ph_1*dx_ph_1
             + dx_ph_2*dx_ph_2
             + dx_ph_3*dx_ph_3
            ) / 2;

   rho.rL = pow(lambda * (  pow(fields->ph_0, 2) + pow(fields->ph_1, 2) 
...
double TAnalyzeLattice::compute_q(TFields *fields, TFields *dotFields, 
                                  ...
                                  double dx_ph_0, double dx_ph_1, double dx_ph_2, double dx_ph_3)
{
   double Ad_ph_0 = fields->A_1 * dx_ph_0;
   double Ad_ph_1 = fields->A_1 * dx_ph_1;
   double Ad_ph_2 = fields->A_1 * dx_ph_2;
   double Ad_ph_3 = fields->A_1 * dx_ph_3;
   ...
   return( -dotFields->ph_0 * (h_00 * Ad_ph_0 + h_01 * Ad_ph_1 + h_02 * Ad_ph_2 + h_03 * Ad_ph_3) +
           ...
           -dotFields->ph_3 * (h_03 * Ad_ph_0 + h_13 * Ad_ph_1 + h_23 * Ad_ph_2 + h_33 * Ad_ph_3) );
}

void TAnalyzeLattice::compute_observables(int i, TFields fields[MAX_NSIDE], TFields dotFields[MAX_NSIDE], TObservable& o)
/* IMPORTANT: Expects correctly normalized derivatives to be ready. */
{
   o.E_x = -dotFields[i].A_1;
   o.E_y = -dotFields[i].A_2;
   o.E_z = -dotFields[i].A_3;

   double dx_ph_0, dx_ph_1, dx_ph_2, dx_ph_3, dx_A_2, dx_A_3;
           
   aux[i].get_derivatives(dx_ph_0, dx_ph_1, dx_ph_2, dx_ph_3, dx_A_2, dx_A_3);

   o.B_x = 0;
   o.B_y = -dx_A_3;
   o.B_z = dx_A_2;

   double ph = 1 - sqr(fields[i].ph_0);
   double trig = 2*acos(fields[i].ph_0)/sqrt(ph);
   ...
   {
      o.theta_x = trig * fields[i].ph_1;
      o.theta_y = trig * fields[i].ph_2;
      o.theta_z = trig * fields[i].ph_3;
   }

   double EU2 = o.E_x * o.E_x + o.E_y * o.E_y + o.E_z * o.E_z;

   double AU2 =   fields[i].A_1 * fields[i].A_1
                + fields[i].A_2 * fields[i].A_2
                + fields[i].A_3 * fields[i].A_3;

   o.rho_E = EU2 / 2;

   double h_00, h_01, h_02, h_11, h_12, h_22;
    
   compute_h(fields[i].ph_0, fields[i].ph_1, fields[i].ph_2, fields[i].ph_3, 
             h_00, h_01, h_02, h_11, h_12, h_22);

   TRho rho_static;
   get_rho_static_core(&(fields[i]), h_00, h_01, h_02, h_11, h_12, h_22,
                       dx_ph_0, dx_ph_1, dx_ph_2, dx_ph_3, dx_A_2, dx_A_3,
                       rho_static);
    
   o.rho_B = rho_static.BR2;

   double rHt = AU2 * (  h_00 * dotFields[i].ph_0 * dotFields[i].ph_0
                       + h_11 * dotFields[i].ph_1 * dotFields[i].ph_1
                       + h_22 * dotFields[i].ph_2 * dotFields[i].ph_2
                       + h_33 * dotFields[i].ph_3 * dotFields[i].ph_3

                       + 2 * (  h_01 * dotFields[i].ph_0 * dotFields[i].ph_1
                              + h_02 * dotFields[i].ph_0 * dotFields[i].ph_2
                              + h_03 * dotFields[i].ph_0 * dotFields[i].ph_3
                              + h_12 * dotFields[i].ph_1 * dotFields[i].ph_2
                              + h_13 * dotFields[i].ph_1 * dotFields[i].ph_3
                              + h_23 * dotFields[i].ph_2 * dotFields[i].ph_3 )
                      );

   if (0 > rHt) rHt = 0; else rHt /= 2;

   double rGt = (  dotFields[i].ph_0 * dotFields[i].ph_0
                 + dotFields[i].ph_1 * dotFields[i].ph_1
                 + dotFields[i].ph_2 * dotFields[i].ph_2
                 + dotFields[i].ph_3 * dotFields[i].ph_3 ) / 2;
   ...
   o.divE = -aux[i].compute_divA(i, nSideM1, dotFields);
   o.q = compute_q(&(fields[i]), &(dotFields[i]),
                   h_00, h_01, h_02, h_11, h_12, h_22, 
                   dx_ph_0, dx_ph_1, dx_ph_2, dx_ph_3);

   double M_11, M_12, M_13, M_22, M_23, M_33;   
   compute_photon_msqr(&(dotFields[i]),
                       h_00, h_01, h_02, h_11, h_12, h_22,
                       dx_ph_0, dx_ph_1, dx_ph_2, dx_ph_3,
...
void TAnalyzeLattice::writeVTKheader(ostream& vtk, string title)
{
   ...
   vtk << "DIMENSIONS " << get_nSide() << endl;
   vtk << "ORIGIN 0" << endl;
   vtk << "SPACING 1" << endl;
   vtk << "POINT_DATA " << get_nSide() << endl;
}
...
#pragma omp for
   ...
   {
      compute_dotFields(&(fields[active][i]), &(momenta[active][i]), dotFields[i]);
   }
...
#pragma omp barrier
   ...
   {
      TObservable o;
      compute_observables(i, fields[active], dotFields, o);

      vtk_A     << fields[active][i].A_1 << ' ' 
                << fields[active][i].A_2 << ' ' 
                << fields[active][i].A_3 << endl;
      ...
      double r = max(max(max(abs(dotFields[i].ph_0),
                             abs(dotFields[i].ph_1)), 
                             abs(dotFields[i].ph_2)), 
                             abs(dotFields[i].ph_3));
      ...
      r = max(max(max(abs(dotFields[i].ph_0 / fields[active][i].ph_0), 
                      abs(dotFields[i].ph_1 / fields[active][i].ph_1)), 
                      abs(dotFields[i].ph_2 / fields[active][i].ph_2)), 
                      abs(dotFields[i].ph_3 / fields[active][i].ph_3));
      ...
      r = max(max(abs(dotFields[i].A_1 / fields[active][i].A_1), 
                  abs(dotFields[i].A_2 / fields[active][i].A_2)), 
                  abs(dotFields[i].A_3 / fields[active][i].A_3));
      ...
      mean.A_x        += fields[active][i].A_1;
      mean.A_y        += fields[active][i].A_2;
      mean.A_z        += fields[active][i].A_3;

      mean.ph_0       += fields[active][i].ph_0;
      mean.ph_1       += fields[active][i].ph_1;
      mean.ph_2       += fields[active][i].ph_2;
      mean.ph_3       += fields[active][i].ph_3;
   }
   ...
   averages << "max dt_phi_a: " << max_dt_phi << " (" << max_dt_phi_i << ")" << endl;
   averages << "max dt_A_a: " << max_dt_A << " (" << max_dt_A_i << ")" << endl;
   averages << "max dt_phi_a/phi_a: " << max_rel_dt_phi << " (" << max_rel_dt_phi_i << ")" << endl;
   averages << "max dt_A_a/A_a: " << max_rel_dt_A << " (" << max_rel_dt_A_i << ")" << endl;
...
#pragma omp for
   ...
   for (int i = 0; i < nSide; i++)
   {
      ...
      var2.A_x        += sqr(fields[active][i].A_1 - mean.A_x);
      var2.A_y        += sqr(fields[active][i].A_2 - mean.A_y);
      var2.A_z        += sqr(fields[active][i].A_3 - mean.A_z);

      var2.ph_0       += sqr(fields[active][i].ph_0 - mean.ph_0);
      var2.ph_1       += sqr(fields[active][i].ph_1 - mean.ph_1);
      var2.ph_2       += sqr(fields[active][i].ph_2 - mean.ph_2);
      var2.ph_3       += sqr(fields[active][i].ph_3 - mean.ph_3);
   }
...
#pragma omp for
   ...
   {
      double E_x_a = -dotFields[i_a].A_1;
      double E_y_a = -dotFields[i_a].A_2;
      double E_z_a = -dotFields[i_a].A_3;

      double B_x_a = 0;
      double B_y_a = -aux[i_a].dx_A_3;
      double B_z_a = aux[i_a].dx_A_2;

      for (int i_b = i_a + 1; i_b < nSide; i_b++)
      {
         double distance = abs(i_b - i_a) % nSideO2;

         int distance_bin = int(distance + 0.5);

         double E_x_b = -dotFields[i_b].A_1;
         double E_y_b = -dotFields[i_b].A_2;
         double E_z_b = -dotFields[i_b].A_3;

         double B_x_b = 0;
         double B_y_b = -aux[i_b].dx_A_3;
         double B_z_b = aux[i_b].dx_A_2;
         ...
         cov[distance_bin].ph_0 += fields[active][i_a].ph_0 * fields[active][i_b].ph_0;
         cov[distance_bin].ph_1 += fields[active][i_a].ph_1 * fields[active][i_b].ph_1;
         cov[distance_bin].ph_2 += fields[active][i_a].ph_2 * fields[active][i_b].ph_2;
         cov[distance_bin].ph_3 += fields[active][i_a].ph_3 * fields[active][i_b].ph_3;

         cov[distance_bin].A_x += fields[active][i_a].A_1 * fields[active][i_b].A_1;
         cov[distance_bin].A_y += fields[active][i_a].A_2 * fields[active][i_b].A_2;
         cov[distance_bin].A_z += fields[active][i_a].A_3 * fields[active][i_b].A_3;
...
#pragma omp barrier
   ...
   for (int i = 0; i < int(nSideO2 + 0.5); i++)
   {
      covariance << i << ", "
