/* analyze_frw_1d.cpp : Analyzes raw (cartesian) field data on FRW metric (1D).
 * 
...
/* Individual terms are returned in the TRho struct.  */
{
   ...
   rho.BR2 = ( pow(dx_A_3, 2) + pow(dx_A_2, 2) ) / twoaR4;
...
{
   o.E_x = -dotFields[i].A_1 / a;
   o.E_y = -dotFields[i].A_2 / a;
   o.E_z = -dotFields[i].A_3 / a;
   ...
   TRho rho_static;
   get_rho_static_core(&(fields[i]), twoaR2, twoaR4,
                       h_00, h_01, h_02, h_11, h_12, h_22,
   ...
   double rGt = (  dotFields[i].ph_0 * dotFields[i].ph_0
                 ...
                 + dotFields[i].ph_3 * dotFields[i].ph_3 ) / twoaR2;
   ...
   o.divE = -aux[i].compute_divA(i, nSideM1, dotFields) / aR2;
...
#pragma omp barrier
   ...
   averages << "max deta_phi_a: " << max_dt_phi << " (" << max_dt_phi_i << ")" << endl;
   averages << "max deta_A_a: " << max_dt_A << " (" << max_dt_A_i << ")" << endl;
   averages << "max deta_phi_a/phi_a: " << max_rel_dt_phi << " (" << max_rel_dt_phi_i << ")" << endl;
   averages << "max deta_A_a/A_a: " << max_rel_dt_A << " (" << max_rel_dt_A_i << ")" << endl;
...
#pragma omp for
   ...
   {
      double E_x_a = -dotFields[i_a].A_1 / a;
      double E_y_a = -dotFields[i_a].A_2 / a;
      double E_z_a = -dotFields[i_a].A_3 / a;

      double B_x_a = 0;
      double B_y_a = -aux[i_a].dx_A_3 / aR2;
      double B_z_a = aux[i_a].dx_A_2 / aR2;
      ...
      for (int i_b = i_a + 1; i_b < nSide; i_b++)
      {
         ...
         double E_x_b = -dotFields[i_b].A_1 / a;
         double E_y_b = -dotFields[i_b].A_2 / a;
         double E_z_b = -dotFields[i_b].A_3 / a;

         double B_x_b = 0;
         double B_y_b = -aux[i_b].dx_A_3 / aR2;
         double B_z_b = aux[i_b].dx_A_2 / aR2;
