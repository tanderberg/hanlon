/* p2c_1d.cpp : Converts vtk in polar form to cartesian, discards momenta (1D).
 * 
...
TFields fields[2][MAX_NSIDE];   /* Max allowed number of fieldss always allocated. Ugly but fast. */
...
void TP2CLattice::writeCartesianVTK(string fileName, string title)
{
   ...
   for (i = 0; i <= nSideM1; i++)
   {
      double theta = sqrt(pow(fields[active][i].th_1, 2) + 
                          pow(fields[active][i].th_2, 2) + 
                          pow(fields[active][i].th_3, 2)); 
   ...
   for (i = 0; i <= nSideM1; i++)
   {
      double thR2 = pow(fields[active][i].th_1, 2) + 
                    pow(fields[active][i].th_2, 2) + 
                    pow(fields[active][i].th_3, 2);

      if (thR2 < SMALL_THETA_SQUARED) // O(th_i^3) series expansion 
      {
         vtk << fields[active][i].th_1 * (24 - thR2) / 48 << endl;
      }
      else
      {
         double theta = sqrt(thR2);
         vtk << (fields[active][i].th_1 / theta) * sin(theta / 2) << endl;
      }
   }
   ...
   for (i = 0; i <= nSideM1; i++)
   {
      ...
      if (thR2 < SMALL_THETA_SQUARED) // O(th_i^3) series expansion 
      {
         vtk << fields[active][i].th_2 * (24 - thR2) / 48 << endl;
      }
      else
      {
         double theta = sqrt(thR2);
         vtk << (fields[active][i].th_2 / theta) * sin(theta / 2) << endl;
      }
   }
   ...
   for (i = 0; i <= nSideM1; i++)
   {
      ...
      if (thR2 < SMALL_THETA_SQUARED) // O(th_i^3) series expansion 
      {
         vtk << fields[active][i].th_3 * (24 - thR2) / 48 << endl;
      }
      else
      {
         double theta = sqrt(thR2);
         vtk << (fields[active][i].th_3 / theta) * sin(theta / 2) << endl;
      }
   }
