/* optimize.cpp : Minimizes local energy density (scalars in polar form).
 * 
...
TFields fields[2][MAX_NSIDE][MAX_NSIDE][MAX_NSIDE];   /* Max allowed number of cells always allocated. Ugly but fast. */
...
double rho_th1(double th1, void * params)
{
   ...
   par->lattice->copy2private(i, j, k);
   par->lattice->privateFields[4][4][4].th_1 = wrap(th1);

   par->lattice->compute_affected_private_derivatives(0);
   par->lattice->privateAux[4][4][4].compute_hG(par->lattice->privateFields[4][4][4].th_1, 
                                                par->lattice->privateFields[4][4][4].th_2, 
                                                par->lattice->privateFields[4][4][4].th_3);
...
double rho_th2(double th2, void * params)
{
   ...
   par->lattice->copy2private(i, j, k);
   par->lattice->privateFields[4][4][4].th_2 = wrap(th2);
...
double rho_th3(double th3, void * params)
{
   ...
   par->lattice->copy2private(i, j, k);
   par->lattice->privateFields[4][4][4].th_3 = wrap(th3);
...
double rho_A1(double A1, void * params)
{
   ...
   par->lattice->copy2private(i, j, k);
   par->lattice->privateFields[4][4][4].A_1 = A1;
...
double rho_A2(double A2, void * params)
{
   ...
   par->lattice->copy2private(i, j, k);
   par->lattice->privateFields[4][4][4].A_2 = A2;
...
double rho_A3(double A3, void * params)
{
   ...
   par->lattice->copy2private(i, j, k);
   par->lattice->privateFields[4][4][4].A_3 = A3;
...
double rho(const gsl_vector *v, void *params)
{
   ...
   par->lattice->privateFields[4][4][4].th_1 = wrap(gsl_vector_get(v, 0));
   par->lattice->privateFields[4][4][4].th_2 = wrap(gsl_vector_get(v, 1));
   par->lattice->privateFields[4][4][4].th_3 = wrap(gsl_vector_get(v, 2));

   par->lattice->privateFields[4][4][4].A_1 = gsl_vector_get(v, 3);
   par->lattice->privateFields[4][4][4].A_2 = gsl_vector_get(v, 4);
   par->lattice->privateFields[4][4][4].A_3 = gsl_vector_get(v, 5);
   ...
   for (int i = from_i; i <= to_i; i++)
   {
       ...
       for (int j = 0; j <= nSideM1; j++)
       {
           ...
           {   
               ...
               gsl_vector_set(v, 0, fields[active][i][j][k].th_1);
               gsl_vector_set(v, 1, fields[active][i][j][k].th_2);
               gsl_vector_set(v, 2, fields[active][i][j][k].th_3);
               gsl_vector_set(v, 3, fields[active][i][j][k].A_1);
               gsl_vector_set(v, 4, fields[active][i][j][k].A_2);
               gsl_vector_set(v, 5, fields[active][i][j][k].A_3);
               ...
               if (iter == max_iter) cout << "Iteration limit hit at " << i << ", " << j << ", " << k << endl << flush;
               
               fields[inActive][i][j][k].th_1 = wrap(gsl_vector_get(s->x, 0));
               fields[inActive][i][j][k].th_2 = wrap(gsl_vector_get(s->x, 1));
               fields[inActive][i][j][k].th_3 = wrap(gsl_vector_get(s->x, 2));
               fields[inActive][i][j][k].A_1 = gsl_vector_get(s->x, 3);
               fields[inActive][i][j][k].A_2 = gsl_vector_get(s->x, 4);
               fields[inActive][i][j][k].A_3 = gsl_vector_get(s->x, 5);
           }
       }
   }
