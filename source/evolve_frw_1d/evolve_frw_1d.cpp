/* evolve_frw_1d.cpp : Euler, RK4 and symplectic (default) integrators (FRW 1D).
 * 
...
class TRunLattice : public TLattice
{
   ...
   double compute_rho(int i, double aR2, double aR4,
                      TFields fields[MAX_NSIDE], 
                      TFields momenta[MAX_NSIDE],
                      bool includeLambdaTerm);
...
public:
   ...
   double compute_rho(TFields fields[MAX_NSIDE], 
                      TFields momenta[MAX_NSIDE],
   bool includeLambdaTerm);
...
#pragma omp for
   ...
   for (int i = 0; i <= nSideM1; i++)
   {
      compute_dots(i, j, k, fields[substep_1], &(momenta[substep_1][i]), dotFields[substep_1][i], dotMomenta[substep_1][i]);
   }
...
#pragma omp for
   ...
   {
      TFields df, dm;
      compute_dots(i, fields[substep_1], &(momenta[substep_1][i]), df, dm);
...
#pragma omp for
      ...
      for (int i = 0; i <= nSideM1; i++)
      {
         compute_dotFields_and_dDotFields_dFields_aR3(&(fields[scratch][i]), &(momenta[active][i]), df, A);
...
#pragma omp for
      ...
      for (int i = 0; i <= nSideM1; i++)
      {
         compute_dotMomenta_and_dDotFields_dFields_aR3(i, fields[scratch], &(momenta[from_p][i]), dm, A);
                                            ...
                                            bool includeLambdaTerm)
{
   ...
   double rho_BR2 = ( pow(dx_A_3, 2) + pow(dx_A_2, 2) ) / twoaR4;
...
double TRunLattice::compute_rho(int i, double aR2, double aR4,
                                TFields fields[MAX_NSIDE], 
                                TFields momenta[MAX_NSIDE],
                                bool includeLambdaTerm)
/* IMPORTANT: Expects derivatives to be ready. */
{
   ...
   double rho_E = ( sqr(momenta[i].A_1) + sqr(momenta[i].A_2) + sqr(momenta[i].A_3) ) / (2 * aR2);

   double AmAmOaR2 = ( sqr(fields[i].A_1) + sqr(fields[i].A_2) + sqr(fields[i].A_3) ) / aR2;

   double h_00, h_01, h_02, h_11, h_12, h_22,
          invM_00, invM_01, invM_02, invM_03, invM_11, invM_12, invM_13, invM_22, invM_23, invM_33;

   compute_invMh(fields[i].ph_0, fields[i].ph_1, fields[i].ph_2, fields[i].ph_3, AmAmOaR2,
                 invM_00, invM_01, invM_02, invM_03, invM_11, invM_12, invM_13, invM_22, invM_23, invM_33,
                 h_00, h_01, h_02, h_11, h_12, h_22);

   double rho_static = compute_rho_static_core(&(fields[i]), 2 * aR2, 2 * aR4,
                                               h_00, h_01, h_02, h_11, h_12, h_22,
                                               dx_ph_0, dx_ph_1, dx_ph_2, dx_ph_3, dx_A_2, dx_A_3,
                                               includeLambdaTerm);

   double rho_M = (  momenta[i].ph_0 * (momenta[i].ph_3*invM_03 + momenta[i].ph_2*invM_02 + momenta[i].ph_1*invM_01 + momenta[i].ph_0*invM_00)
                   + momenta[i].ph_1 * (momenta[i].ph_3*invM_13 + momenta[i].ph_2*invM_12 + momenta[i].ph_1*invM_11 + momenta[i].ph_0*invM_01)
                   + momenta[i].ph_2 * (momenta[i].ph_3*invM_23 + momenta[i].ph_2*invM_22 + momenta[i].ph_1*invM_12 + momenta[i].ph_0*invM_02)
                   + momenta[i].ph_3 * (momenta[i].ph_3*invM_33 + momenta[i].ph_2*invM_23 + momenta[i].ph_1*invM_13 + momenta[i].ph_0*invM_03) 
                  ) / (2 * aR4);

   return(rho_static + rho_E + rho_M);
}

double TRunLattice::compute_rho(TFields fields[MAX_NSIDE], 
                                TFields momenta[MAX_NSIDE],
                                bool includeLambdaTerm)
/* IMPORTANT: Expects derivatives to be ready. */
{
...
#pragma omp for
   ...
   for (int i = 0; i <= nSideM1; i++)
   {
      rho += compute_rho(i, aR2, aR4, fields, momenta, includeLambdaTerm);
   }
#ifdef _OPENMP
#pragma omp barrier
}
...
#pragma omp for
   ...
   for (int i = 0; i <= nSideM1; i++)
   {
      fields[active][i].A_1 *= a;
      fields[active][i].A_2 *= a;
      fields[active][i].A_3 *= a;

      momenta[active][i].ph_0 *= a;
      momenta[active][i].ph_1 *= a;
      momenta[active][i].ph_2 *= a;
      momenta[active][i].ph_3 *= a;
   }
#ifdef _OPENMP
#pragma omp barrier
}
